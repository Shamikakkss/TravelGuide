<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
    ...
    <div v-if="!currentUser" class="p-8 sm:p-12 text-center glass-panel rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4 max-w-lg mx-auto">
      ...
      <h3 class="text-xl font-bold text-slate-900 dark:text-white">Member Dashboard</h3>
      <p class="text-xs text-slate-500 leading-relaxed">
        Log in to view your submitted attractions, track approval statuses, and access your saved favorites.
      </p>
      <div class="pt-2 flex justify-center gap-3">
        <button 
          @click="openAuthModal('login', 'Please log in to access your dashboard.')"
          class="px-5 py-2.5 rounded-xl bg-brand-500 text-white font-bold text-xs shadow-md shadow-brand-500/20"
        >
          Log In
        </button>
        <button 
          @click="openAuthModal('register', 'Create an account to access the dashboard.')"
    ...
    <!-- User Profile Banner (When Logged In) -->
...
const { currentUser, submissions, places, stays, reviews, likedPlaceIds, likedStayIds, openAuthModal } = useTravelData()
